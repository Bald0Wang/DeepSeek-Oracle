...
export default function LoginPage({ onAuthSuccess }: LoginPageProps) {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const res = await loginByEmail({ email: email.trim(), password: password.trim() });
      if (!res.data) {
        throw new Error("登录失败");
      }
      setAuthData(res.data.token, res.data.user);
      onAuthSuccess?.(res.data.user);
      navigate(res.data.user.role === "admin" ? "/admin" : "/oracle", { replace: true });
    } catch (err) {
    ...
    <div className="auth-page fade-in">
      <InkCard title="邮箱登录" icon="登">
        <form className="stack" onSubmit={onSubmit}>
          ...
          <div className="field field--inline-end">
            <Link className="text-link" to="/forgot-password">忘记密码？</Link>
          </div>
